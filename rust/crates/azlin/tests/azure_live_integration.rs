...
fn test_live_list_json() {
    if !has_azure_login() { return; }
    let (stdout, _, code) = run_azlin(&["list", "--no-tmux", "--resource-group", RG, "--output", "json"]);
    assert_eq!(code, 0);
...
fn test_live_list_csv() {
    if !has_azure_login() { return; }
    let (stdout, _, code) = run_azlin(&["list", "--no-tmux", "--resource-group", RG, "--output", "csv"]);
    assert_eq!(code, 0);
    let lines: Vec<&str> = stdout.lines().collect();
    assert!(lines.len() >= 2, "CSV should have header + data");
    assert!(lines[0].contains("Session"), "CSV header should contain Session");
}
...
fn test_live_list_compact() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("Total:"), "compact should still have footer");
}
...
fn test_live_list_bastion_table() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("Azure Bastion Hosts"), "should show bastion table");
}
...
fn test_live_tag_roundtrip() {
    if !has_azure_login() { return; }
    // Add
    let (_, _, code) = run_azlin(&["tag", "add", VM, "integration-test=live", "--resource-group", RG]);
    ...
    // Remove
    let (_, _, code) = run_azlin(&["tag", "remove", VM, "integration-test", "--resource-group", RG]);
...
fn test_live_health() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("Four Golden Signals"), "should show health header");
    assert!(stdout.contains(VM), "should show VM name");
    ...
    assert!(stdout.contains("Thresholds:"), "should show thresholds footer");
}
...
fn test_live_w() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("load average"), "w should show load average");
}
...
fn test_live_ps() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("PID") || stdout.contains("COMMAND"), "ps should show process info");
}
...
fn test_live_config_show() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("az_cli_timeout"), "config should show timeout");
}
...
fn test_live_bastion_list() {
    ...
    assert_eq!(code, 0);
    assert!(stdout.contains("Bastion") || stdout.contains("bastion"), "should list bastions");
}
