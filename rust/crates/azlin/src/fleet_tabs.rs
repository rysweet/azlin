...
struct FleetTabsState {
    outputs: Vec<VmOutput>,
    active_tab: usize,
    scroll_offset: u16,
    show_summary: bool,
}
...
impl FleetTabsState {
    fn new(outputs: Vec<VmOutput>) -> Self {
        Self {
            outputs,
            active_tab: 0,
            scroll_offset: 0,
    ...
    fn next_tab(&mut self) {
        if !self.outputs.is_empty() {
            self.active_tab = (self.active_tab + 1) % self.outputs.len();
...
fn render_tabs(f: &mut ratatui::Frame, state: &FleetTabsState) {
    ...
    let titles: Vec<Line> = state
        ...
        .map(|(i, vm)| {
            ...
            Line::from(vec![
                Span::styled(format!(" {} ", vm.status_icon()), Style::default().fg(c)),
                Span::raw(format!("{}: {} ", num, vm.vm_name)),
            ])
        })
        .collect();
    ...
    if state.show_summary {
        let mut lines = vec![
            Line::from(vec![
                Span::styled(
                    format!(
                        " {} ok ",
                        state.outputs.iter().filter(|v| v.succeeded()).count()
                    ),
        ...
        for vm in &state.outputs {
            ...
            lines.push(Line::from(vec![
                ...
                Span::raw(format!("{:<20} exit: {}", vm.vm_name, vm.exit_code)),
            ]));
        }
        f.render_widget(
            Paragraph::new(lines)
        ...
        let status = if vm.succeeded() {
            "success".into()
        } else {
            format!("exit {}", vm.exit_code)
        };
        f.render_widget(
            Paragraph::new(vm.combined_output())
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(format!(" {} - {} ", vm.vm_name, status)),
                )
                .wrap(Wrap { trim: false })
    ...
    // Footer
    let toggle = if state.show_summary {
        "output"
    } else {
        "summary"
    };
    f.render_widget(
        Block::default()
            .title(format!(
                " q: quit | <-/->: tabs | 1-9: jump | s: {} | j/k: scroll ",
                toggle
            ))
            .borders(Borders::ALL)
...
pub fn print_plain_output(outputs: &[VmOutput]) {
    ...
    println!(
        "Summary: {} succeeded, {} failed out of {} VMs",
        ok,
        outputs.len() - ok,
        outputs.len()
    );
...
mod tests {
    ...
    #[test]
    fn test_succeeded() {
        assert!(sample()[0].succeeded());
        assert!(!sample()[1].succeeded());
    }
    #[test]
    fn test_combined_output() {
        assert!(sample()[2].combined_output().contains("out"));
        assert!(sample()[2].combined_output().contains("warn"));
    }
    #[test]
    fn test_status_icon() {
        assert_eq!(sample()[0].status_icon(), "\u{2713}");
        assert_eq!(sample()[1].status_icon(), "\u{2717}");
    }
    #[test]
    fn test_navigation() {
    ...
    fn test_scroll() {
        ...
        s.scroll_up();
        assert_eq!(s.scroll_offset, 0); // no underflow
    }
}
