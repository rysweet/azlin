...
| `azlin list --show-tmux` | +1-3s | 1 per VM | No | Use `--no-tmux` (shorthand for `--show-tmux false`) for speed |
...
4. If tmux collection is on (the default; `--show-tmux false`, or its shorthand `--no-tmux`, turns it off): SSH to each running VM (0.5-1s each, parallel max 10)
