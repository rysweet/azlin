"""Snapshot commands for azlin CLI.

This module contains all snapshot-related commands extracted from cli.py
as part of Issue #423 - CLI decomposition.

Commands:
    snapshot         - Group command for VM snapshot management
    snapshot enable  - Enable scheduled snapshots for a VM
    snapshot disable - Disable scheduled snapshots for a VM
    snapshot sync    - Sync snapshots for VMs with schedules
    snapshot status  - Show snapshot schedule status for a VM
    snapshot create  - Create a snapshot of a VM's OS disk
    snapshot list    - List all snapshots for a VM
    snapshot restore - Restore a VM from a snapshot
...
# Export the snapshot group for registration in cli.py
__all__ = ["snapshot"]
