"""Provisioning commands for azlin.

This module contains VM provisioning CLI commands extracted from cli.py.
...
# Import from connectivity module (same decomposition)
from azlin.commands.connectivity import _auto_sync_home_directory
...
def _display_clone_results(
    ...
    for vm in result.successful:
        ...
        copy_status = "[OK]" if copy_results.get(vm.name, False) else "[FAIL]"
    ...
    if result.failed:
        click.echo("\nFailed provisioning:")
        for failure in result.failed:
            click.echo(f"  [FAIL] {failure.config.name}: {failure.error}")
    ...
    def copy_to_vm(clone_vm: VMDetails) -> tuple[str, bool]:
        ...
        try:
            ...
            if result1.returncode != 0:
                click.echo(
                    f"  [FAIL] {clone_vm.name} download failed: {result1.stderr[:100]}", err=True
                )
            ...
            if result2.returncode == 0:
                click.echo(f"  [OK] {clone_vm.name} copy complete")
                return (clone_vm.name, True)
            click.echo(f"  [FAIL] {clone_vm.name} upload failed: {result2.stderr[:100]}", err=True)
            return (clone_vm.name, False)

        except subprocess.TimeoutExpired:
            click.echo(f"  [FAIL] {clone_vm.name} copy timeout", err=True)
            return (clone_vm.name, False)
        except Exception as e:
            click.echo(f"  [FAIL] {clone_vm.name} copy error: {e}", err=True)
            return (clone_vm.name, False)
        finally:
...
def new_command(
    ...
    """
    # Import CLIOrchestrator here to avoid circular imports
    from azlin.cli import CLIOrchestrator
