"""Batch operations on multiple VMs for azlin.

This module contains batch operation CLI commands extracted from cli.py.
Part of Issue #423 - cli.py decomposition.

Commands:
    - batch: Command group for batch operations
    - batch stop: Batch stop/deallocate VMs
    - batch start: Batch start VMs
    - batch command: Execute command on multiple VMs
    - batch sync: Batch sync home directory to VMs
"""
...
def batch_stop(
    ...
    try:
        ...
        running_vms = [vm for vm in selected_vms if vm.is_running()]
        if not running_vms:
            click.echo(f"No running VMs found matching {selection_desc}.")
            return

        # Show summary
        action = "deallocate" if deallocate else "stop (keep allocated)"
        click.echo(f"\nFound {len(running_vms)} running VM(s) matching {selection_desc}:")
        click.echo("=" * 80)
        ...
        # Confirmation
        if not _confirm_batch_operation(len(running_vms), action, confirm):
            return

        # Execute batch stop
        click.echo(f"\nStopping {len(running_vms)} VM(s)...")
        ...
        results = executor.execute_stop(
            running_vms, rg, deallocate=deallocate, progress_callback=progress_callback
        )

        # Show results
        batch_result = BatchResult(results)
        _display_batch_summary(batch_result, "Stop")
...
# =============================================================================
# Batch Command (Execute Shell Commands)
# =============================================================================
