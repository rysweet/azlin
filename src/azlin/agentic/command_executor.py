...
class CommandExecutor:
    ...
    def execute_workflow(
        ...
        """Execute commands with progress reporting and enhanced error handling.

        This method groups commands and provides progress feedback, making it
        suitable for multi-step workflows. Errors are enhanced with actionable
        suggestions using ErrorAnalyzer.

        Args:
            commands: List of command specs with 'command' and 'args' keys
            progress_callback: Optional callback called with (current, total, description)
                             for progress reporting
            stop_on_error: Whether to stop execution on first error (default: True)

        Returns:
            List of execution results with enhanced error messages
        ...
        for idx, cmd_spec in enumerate(commands, start=1):
            # Build command description for progress
            command = cmd_spec["command"]
            args = cmd_spec.get("args", [])
            args_str = " ".join(args) if isinstance(args, list) else str(args)
            description = f"Executing {command} {args_str}".strip()

            # Report progress before execution
            if progress_callback:
                progress_callback(idx, total, description)

            # Execute command
            result = self.execute(cmd_spec)

            # Enhance error message if command failed
            if not result["success"] and result.get("stderr"):
                full_command = f"{command} {args_str}".strip()
                enhanced_error = analyzer.analyze(full_command, result["stderr"])
                # Store both original and enhanced error
                result["error_enhanced"] = enhanced_error  # type: ignore[typeddict-unknown-key]
