...
class ArtifactGenerator:
    ...
    def generate_all(
        self, hierarchy: GoalHierarchy, state: ExecutionState
    ) -> dict[str, Path]:
    ...
    def generate_terraform(self, hierarchy: GoalHierarchy) -> Path | None:
        ...
        completed = [
            g for g in hierarchy.goals if g.status == GoalStatus.COMPLETED
        ]
...
'''
    ...
    def generate_terraform_variables(
        ...
        location = list(locations)[0] if locations else "eastus"
        environment = list(environments)[0] if environments else "dev"
...
"""
        ...
        has_apim = any(
            g.name.startswith("apim") and g.status == GoalStatus.COMPLETED
            for g in hierarchy.goals
        )
        has_app = any(
            g.name.startswith("app") and g.status == GoalStatus.COMPLETED
            for g in hierarchy.goals
        )
        has_cosmos = any(
        ...
        has_kv = any(
            g.name.startswith("kv") and g.status == GoalStatus.COMPLETED
            for g in hierarchy.goals
        )
        has_storage = any(
            g.name.startswith("st") and g.status == GoalStatus.COMPLETED
            for g in hierarchy.goals
        )
    ...
    def _generate_cost_estimates(self, hierarchy: GoalHierarchy) -> str:
        ...
        for goal in hierarchy.goals:
            ...
            if "app-service-plan" in goal.name or "plan-" in goal.name:
                ...
                cost = (
                    {"Developer": "$50", "Standard": "$680", "Premium": "$2,800"}.get(
                        sku, "$50"
                    )
                )
