...
fail due to transient errors (network timeouts, Azure throttling, rate limiting).

Design Philosophy:
- Ruthless simplicity: Single decorator for all retry needs
- Zero-BS: No over-abstraction, just working retry logic
- Rate limiting aware: Special handling for HTTP 429 with Retry-After
...
from azlin.rate_limiter import handle_rate_limit_error, is_rate_limit_error
...
# Secure random generator for jitter
...
def retry_with_exponential_backoff(
    ...
    - Does not count against max_attempts for initial rate limits
            ...
            while attempt < max_attempts:
                ...
                try:
                    ...
                    if is_rate_limit_error(e):
                        ...
                        wait_time = handle_rate_limit_error(e, default_backoff=delay)

                        if wait_time > 0:
                            logger.warning(
                                f"{func.__name__} rate limited (429), "
                                f"waiting {wait_time:.2f}s (attempt {attempt}/{max_attempts})"
                            )
                            time.sleep(wait_time)

                            # Don't count first rate limit against max_attempts
                            # (Azure rate limits are not user's fault)
                            if attempt == 1:
                                attempt = 0  # Reset counter for first rate limit
                                delay = initial_delay
                            continue
                    ...
                    if jitter:
                        ...
                        actual_delay = delay + _random.uniform(-jitter_amount, jitter_amount)
