...
from .exceptions import InvalidSessionNameError, SessionNotFoundError, MultipleSessionsError
...
class SessionManager:
    ...
    def validate_session_name(cls, session_name: str) -> str:
        ...
        if len(session_name) < MIN_SESSION_NAME_LENGTH:
            raise InvalidSessionNameError(
                f"Session name too short (min {MIN_SESSION_NAME_LENGTH})"
            )

        if len(session_name) > MAX_SESSION_NAME_LENGTH:
            raise InvalidSessionNameError(
                f"Session name too long (max {MAX_SESSION_NAME_LENGTH})"
            )
    ...
    def parse_session_path(cls, arg: str) -> tuple[Optional[str], str]:
        ...
        if len(parts) != 2:
            raise InvalidSessionNameError(
                "Invalid session:path format. Use 'session:path'"
            )
    ...
    def get_vm_session(
        ...
        resource_group = config_manager.get_default_resource_group()
        ...
        matching_vms = [
            vm for vm in vms
            if vm.name == session_name or vm.name.startswith(session_name)
        ]

        if len(matching_vms) == 0:
            raise SessionNotFoundError(
                f"No VM found matching session '{session_name}'"
            )
        ...
        if vm.power_state != "running":
            raise SessionNotFoundError(
                f"VM '{vm.name}' is not running (state: {vm.power_state})"
            )

        # Validate IP exists
        if not vm.public_ip:
            raise SessionNotFoundError(
                f"VM '{vm.name}' has no public IP"
            )
