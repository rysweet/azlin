...
class NotificationHandler:
    ...
    def send_notification(cls, message: str, recipient: str | None = None) -> NotificationResult:
        ...
        if not cls.is_imessr_available():
            ...
            return NotificationResult(sent=False, message="imessR not available", error=None)
        ...
        try:
            ...
            return NotificationResult(sent=False, message=message, error="Timeout")

        except Exception as e:
            logger.warning(f"Notification failed: {type(e).__name__}: {e}")

            return NotificationResult(sent=False, message=message, error=str(e))
    ...
    def send_completion_notification(
        cls, vm_name: str, vm_ip: str, success: bool = True
    ) -> NotificationResult:
        """
        ...
        """
        if success:
            message = f"azlin: VM {vm_name} ready at {vm_ip}"
        else:
            message = f"azlin: VM {vm_name} provisioning failed"
...
def notify_completion(vm_name: str, vm_ip: str) -> bool:
    ...
    """
    result = NotificationHandler.send_completion_notification(vm_name, vm_ip, success=True)
    return result.sent
