                ...
                if func_match:
                    ...
                    if found_cmd_name == cmd_name or func_name == cmd_name:
                        ...
                        while k < len(lines):
                            ...
                            if k > j and (
                                "@main.command" in lines[k] or "main.add_command" in lines[k]
                            ):
                                break
                    ...
                    while k < len(lines):
                        ...
                        if current_line.strip():
                            ...
                            if (
                                ...
                                and not current_line.strip().startswith(")")
                                and (
                                    current_line.startswith("def ")
                                    or current_line.startswith("@")
                                    or current_line.startswith("class ")
                                )
                            ):
                                break
                        k += 1
...
def create_module(module_name: str, commands: list[str], cli_path: Path):
    ...
    lines.append("")
    lines.append("Extracted from cli.py as part of Issue #423 decomposition.")
    lines.append('"""')
    lines.append("")
    ...
    print("2. Test the extracted commands")
    print("3. Update cli.py to import and register these commands")
