...
## Azlin VM Provisioning and Storage Architecture Investigation (2026-01-11)

**Date**: 2026-01-11
**Issue**: #514 (Separate /home disk feature)
**Branch**: feat/issue-514-separate-home-disk

**Context**: Investigated azlin's VM provisioning flow, disk configuration, cloud-init generation, and storage architecture to design implementation for separate /home disk feature.

### Key Findings

**1. No Azure Managed Disk Operations Currently Exist**

azlin has **ZERO** Azure Managed Disk code:
- VMs created with implicit OS disk only (`az vm create`)
- No disk creation, attachment, formatting, or mounting operations
- Storage module uses Azure Files NFS, not block storage
- Separate /home disk feature requires **net-new capability pattern**

**2. OS Disk Configuration is Entirely Implicit**

No explicit OS disk flags passed to Azure CLI:
- Size: Based on image (~30GB for Ubuntu)
- Type: Standard_LRS (Standard SSD) by default
- Name: Auto-generated by Azure
- Location: `vm_provisioning.py:564-607` (az vm create command)

**3. Cloud-Init Uses F-String Template Generation**

No external template engine (Jinja2, etc.):
- Method: `_generate_cloud_init()` in `vm_provisioning.py:711-868`
- Variables: Only `ssh_public_key` (optional)
- Format: YAML with `packages` and `runcmd` sections
- Extension point: Add disk setup commands to `runcmd`

**4. VMConfig Dataclass Structure**

Located at `vm_provisioning.py:40-54`:
```python
@dataclass
class VMConfig:
    name: str                    # Required
...
No disk-related fields currently exist.

**5. NFS Storage Architecture**

Uses Azure Files NFS (not managed disks):
- Replaces entire `/home/azureuser` directory
- Mount via `az vm run-command invoke` (bypasses SSH)
- Different pattern than block storage attachment
- Specified with `--nfs-storage` flag

**6. CLI Argument Flow**

```
CLI flags (cli.py:2759) → CLIOrchestrator →
provisioner.create_vm_config() → VMConfig →
provision_vm() → Azure CLI command
```

Integration points for new flags:
- CLI decorators: `cli.py:2740-2776`
- Orchestrator init: `cli.py:188`
- VM config creation: `cli.py:845`

### Implementation Architecture

**Required Components:**

1. **VMConfig Extension** - Add `home_disk_enabled`, `home_disk_size_gb`, `home_disk_sku` fields
2. **New Methods** - `_create_managed_disk()`, `_attach_disk_to_vm()` in VMProvisioner
3. **Cloud-Init Enhancement** - Add disk formatting/mounting commands to `runcmd`
4. **CLI Flags** - `--home-disk-size`, `--no-home-disk` in `azlin new`

**Provisioning Flow Changes:**

```python
VM Creation → Attach Data Disk → Cloud-Init Formats/Mounts → VM Ready
```

**Critical Design Decision:**
- Disk attachment happens BEFORE cloud-init runs
- Format/mount operations in cloud-init (after disk attached)
- NFS and home disk are **mutually exclusive** options

### Key Learnings

1. **Net-New Pattern Required**: No existing disk operations to follow
2. **Cloud-Init Timing**: Disk must be attached during provisioning, formatted in cloud-init
3. **NFS vs Block Storage**: Two completely separate approaches (shared files vs attached disks)
4. **Implicit Defaults**: Azure applies sensible defaults when explicit config omitted
5. **F-String Simplicity**: No template engine needed for cloud-init generation

### Files Analyzed

**Core Files:**
- `src/azlin/cli.py` (354KB) - CLI entry points
- `src/azlin/vm_provisioning.py` (37KB) - Provisioning logic and VMConfig
- `src/azlin/template_manager.py` (10KB) - Cloud-init generation
- `src/azlin/modules/storage_manager.py` (29KB) - Storage operations
- `src/azlin/modules/nfs_provisioner.py` (39KB) - NFS provisioning

**Supporting Files:**
- `tests/unit/test_vm_provisioning_cloud_init.py` - Cloud-init test patterns

### Parallel Investigation Success

Used INVESTIGATION_WORKFLOW.md 6-phase approach:
- **Phase 1**: Scope Definition (6 questions to answer)
- **Phase 2**: Exploration Strategy (architect agent designed plan)
- **Phase 3**: Parallel Deep Dives (4 analyzer agents simultaneously)
- **Phase 4**: Skipped (sufficient information from Phase 3)
- **Phase 5**: Synthesis (comprehensive architecture understanding)
- **Phase 6**: Knowledge Capture (this entry)

**Parallel Agent Deployment** (Wave 1):
1. CLI Architecture Analyst → `cli.py` analysis
2. Provisioning Flow Analyst → `vm_provisioning.py` analysis
3. Cloud-Init Specialist → Template system analysis
4. Storage Architecture Analyst → Storage + NFS analysis

**Time Efficiency**: ~30 minutes for complete system understanding vs estimated 2-3 hours sequential

### Impact

This investigation enables confident implementation of Issue #514:
- Clear understanding of all integration points
- Identified net-new pattern requirement
- Designed architecture that fits existing patterns
- Documented for future storage-related features

### Related

- **Issue**: #514 (Separate /home disk for azlin new)
- **Strategy Document**: `.claude/docs/VM_PROVISIONING_EXPLORATION_STRATEGY.md`
- **Workflow**: INVESTIGATION_WORKFLOW.md (6-phase investigation)
- **Pattern**: Parallel Agent Investigation (4 agents simultaneously)

### Philosophy Alignment

✅ **Ruthless Simplicity**: F-string templates, implicit defaults
✅ **Modular Design**: Clear separation (VM creation, disk operations, cloud-init)
✅ **Zero-BS**: No existing disk code (no stubs or placeholders)
