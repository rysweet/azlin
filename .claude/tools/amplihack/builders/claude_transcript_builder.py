...
"""
    ...
    def _extract_outcomes(self, messages: List[Dict[str, Any]]) -> List[str]:
        ...
        for msg in messages:
            ...
            for pattern in outcome_patterns:
                ...
                outcomes.extend(match.strip() for match in matches if len(match) > 5)
    ...
    def _extract_knowledge_artifacts(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, msg in enumerate(messages):
            ...
            artifacts.extend({"type": "file_path", "content": path, "message_index": i} for path in file_paths if len(path) > 5)
