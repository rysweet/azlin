...
class MockGitHubCLI:
    ...
    def auth_login(self, protocol: str = 'https') -> Mock:
        ...
        return Mock(
            returncode=0,
            stdout=f'✓ Logged in as {self.username}',
            stderr=''
        )
    ...
    def repo_clone(self, repo_url: str, directory: Optional[str] = None) -> Mock:
        ...
        return Mock(
            returncode=0,
            stdout=f'Cloning into \'{repo_url.split("/")[-1]}\'...\nDone.',
            stderr=''
        )
    ...
    def handle_command(self, cmd: List[str], **kwargs) -> Mock:
        ...
        if 'auth status' in cmd_str:
            ...
            for i, arg in enumerate(cmd):
                if not arg.startswith('-') and 'gh' not in arg and 'repo' not in arg and 'clone' not in arg:
                    if repo_url is None:
                        repo_url = arg
                    else:
                        directory = arg
            ...
            repo = None
            for arg in cmd:
                if not arg.startswith('-') and 'gh' not in arg and 'repo' not in arg and 'view' not in arg:
                    repo = arg
                    break
