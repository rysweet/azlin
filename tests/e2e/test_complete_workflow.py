...
class TestCompleteWorkflowWithRepo:
    ...
    def test_full_workflow_with_repo_flag(
        ...
        with (
            ...
            assert any(
                "gh repo clone" in str(call) for call in mock_gh_cli_authenticated.mock_calls
            )
    ...
    @patch(
        "sys.argv",
        ["azlin", "--repo", "https://github.com/user/dotfiles", "--vm-size", "Standard_D4s_v3"],
    )
    def test_workflow_with_custom_vm_size(self, mock_azure_credentials, mock_azure_compute_client):
        ...
        with (
            ...
            assert (
                "Standard_D4s_v3" in str(vm_params)
                or vm_params.get("properties", {}).get("hardwareProfile", {}).get("vmSize")
                == "Standard_D4s_v3"
            )
...
class TestCompleteWorkflowWithoutRepo:
    ...
    def test_full_workflow_without_repo_flag(
        self, mock_azure_credentials, mock_azure_compute_client, mock_subprocess_success
    ):
...
class TestWorkflowWithExistingVM:
    ...
    def test_connects_to_existing_vm(self, mock_azure_credentials, mock_azure_compute_client):
...
class TestWorkflowFailureScenarios:
    ...
    @patch("sys.argv", ["azlin"])
    def test_workflow_handles_authentication_failure(self, mock_azure_credentials):
    ...
    def test_workflow_handles_vm_provisioning_failure(
        self, mock_azure_credentials, mock_azure_compute_client
    ):
        ...
        with patch("azlin.progress.ProgressDisplay") as mock_progress:
            ...
            assert any(
                "quota" in str(call).lower() for call in mock_progress.return_value.error.mock_calls
            )

    @patch("sys.argv", ["azlin", "--repo", "https://github.com/user/private-repo"])
    def test_workflow_handles_github_clone_failure(
        self, mock_azure_credentials, mock_azure_compute_client, mock_gh_cli_not_installed
    ):
...
class TestWorkflowProgressTracking:
    ...
    @patch("sys.argv", ["azlin"])
    def test_shows_progress_for_each_step(self, mock_azure_credentials, mock_azure_compute_client):
    ...
    def test_shows_final_completion_message(
        ...
        with (
            ...
            assert mock_progress.return_value.complete.called or any(
                "complete" in str(call).lower() or "success" in str(call).lower()
...
class TestWorkflowNotifications:
    ...
    def test_sends_success_notification_on_completion(
        self, mock_azure_credentials, mock_azure_compute_client, mock_imessr_client
    ):
