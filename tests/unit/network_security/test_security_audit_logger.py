...
class TestSecurityAuditLoggerInitialization:
    ...
    def test_logger_creates_audit_directory(self, mock_chmod, mock_mkdir):
        ...
        assert any(
            call[0][1] == 0o700 for call in mock_chmod.call_args_list
        )
...
class TestSecurityAuditLoggerEventLogging:
    ...
    def test_log_event_sets_secure_permissions(self, mock_chmod, mock_file):
        ...
        assert any(
            call[0][1] == 0o600 for call in mock_chmod.call_args_list
        )
...
class TestSecurityAuditLoggerIntegrityVerification:
    ...
    def test_verify_integrity_detects_corrupted_events(self, mock_file, mock_exists):
        ...
        with patch.object(
            logger, "_compute_checksum", return_value="different-checksum"
        ):
...
class TestSecurityAuditLoggerComplianceReporting:
    ...
    def test_generate_compliance_report_filters_by_framework(
    ...
    def test_generate_compliance_report_counts_critical_findings(
        self, mock_file, mock_exists
    ):
    ...
    def test_generate_compliance_report_counts_policy_violations(
        self, mock_file, mock_exists
    ):
