...
class TestAzureCredentialDetection:
    ...
    def test_detects_az_cli_credentials(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0,
            stdout='{"accessToken": "token123", "expiresOn": "2024-12-31"}',
            stderr=''
        )
    ...
    @patch('os.environ', {'AZURE_CLIENT_ID': 'client-id', 'AZURE_CLIENT_SECRET': 'secret', 'AZURE_TENANT_ID': 'tenant-id'})
    ...
    def test_credential_detection_priority_order(self):
        ...
        with patch.dict('os.environ', {'AZURE_CLIENT_ID': 'client-id', 'AZURE_CLIENT_SECRET': 'secret', 'AZURE_TENANT_ID': 'tenant-id'}):
            auth = AzureAuthenticator()
            credentials = auth.get_credentials()
