...
class TestAzureAuthenticationWorkflow:
    ...
    def test_authentication_with_subscription_detection(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.return_value = Mock(
                returncode=0,
                stdout='{"id": "sub-12345", "name": "My Subscription"}',
                stderr=''
            )
    ...
    def test_authentication_fails_gracefully_with_helpful_error(self):
        ...
        with patch('azure.identity.DefaultAzureCredential') as mock_cred:
            ...
            assert 'az login' in str(exc_info.value).lower() or 'credentials' in str(exc_info.value).lower()
...
class TestVMProvisioningWorkflow:
    ...
    def test_full_vm_creation_with_network_resources(self):
        ...
        with patch('azure.mgmt.compute.ComputeManagementClient', return_value=mock_env['compute_client']), \
             patch('azure.mgmt.network.NetworkManagementClient', return_value=mock_env['network_client']), \
             patch('azure.mgmt.resource.ResourceManagementClient', return_value=mock_env['resource_client']):
    ...
    def test_vm_provisioning_handles_dependencies_correctly(self):
        ...
        with patch('azure.mgmt.resource.ResourceManagementClient', return_value=mock_env['resource_client']) as mock_resource, \
             patch('azure.mgmt.network.NetworkManagementClient', return_value=mock_env['network_client']) as mock_network, \
             patch('azure.mgmt.compute.ComputeManagementClient', return_value=mock_env['compute_client']) as mock_compute:

            # Track call order
            mock_resource.return_value.resource_groups.create_or_update.side_effect = track_call('resource_group')
            mock_network.return_value.virtual_networks.begin_create_or_update.side_effect = track_call('vnet')
            mock_network.return_value.public_ip_addresses.begin_create_or_update.side_effect = track_call('public_ip')
            mock_network.return_value.network_interfaces.begin_create_or_update.side_effect = track_call('nic')
            mock_compute.return_value.virtual_machines.begin_create_or_update.side_effect = track_call('vm')
...
class TestAzureErrorHandling:
    ...
    def test_handles_transient_azure_errors_with_retry(self):
        ...
        with patch('azure.mgmt.compute.ComputeManagementClient', return_value=mock_env['compute_client']) as mock_compute:
            mock_compute.return_value.virtual_machines.begin_create_or_update.side_effect = failing_then_success
...
class TestResourceCleanup:
    ...
    def test_cleans_up_all_resources_on_failure(self):
        ...
        with patch('azure.mgmt.compute.ComputeManagementClient', return_value=mock_env['compute_client']) as mock_compute, \
            ...
            mock_compute.return_value.virtual_machines.begin_delete.side_effect = track_cleanup('vm')
            mock_network.return_value.network_interfaces.begin_delete.side_effect = track_cleanup('nic')
            mock_network.return_value.public_ip_addresses.begin_delete.side_effect = track_cleanup('public_ip')
