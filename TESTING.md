...
| `session_integration.rs` | tmux session persistence |
| `e2e_integration.rs` | End-to-end command flows |
| `parity_integration.rs` | Python/Rust output parity |
| `error_integration.rs` | Error handling and reporting |
| `output_format_integration.rs` | Output format validation |
| `lifecycle_integration.rs` | VM start/stop/delete |
| `restore_integration.rs` | Session/VM restoration |
| `doit_integration.rs` | Agentic "do" command |
| `backup_dr_integration.rs` | Backup and disaster recovery |
| `bastion_integration.rs` | Bastion host connectivity |
| `multi_vm_integration.rs` | Multi-VM operations |
| `home_disk_integration.rs` | Home directory disk management |
| `storage_integration.rs` | Storage operations |
| `template_integration.rs` | VM template operations |
| `local_e2e.rs` | Local end-to-end tests (no Azure) |
...
| `AZLIN_CONFIG_DIR` | Custom config directory (used in config and local E2E tests) | Some integration tests |
| `AZLIN_TEST_MODE` | Enables mock data in list commands | Some unit tests |
| `AZLIN_NO_UPDATE_CHECK` | Suppresses update check during tests (set to `1`) | Some unit tests |
