...
from ..integrator import Integrator, BranchInfo, IntegrationSummary
...
class TestIntegrator(unittest.TestCase):
    ...
    def test_import_branches(self):
        ...
        subprocess.run(['git', 'checkout', '-b', 'feature'], cwd=self.repo_path, check=True, capture_output=True)
        feature_file = self.repo_path / 'feature.txt'
        feature_file.write_text('feature content')
        subprocess.run(['git', 'add', 'feature.txt'], cwd=self.repo_path, check=True)
        subprocess.run(['git', 'commit', '-m', 'Add feature'], cwd=self.repo_path, check=True, capture_output=True)
    ...
    def test_detect_conflicts_none(self):
        ...
        branches = [
            BranchInfo(name='feature', commit='abc123', is_new=True)
        ]
    ...
    def test_create_summary_report_with_conflicts(self):
        ...
        summary = IntegrationSummary(
            branches=[
                BranchInfo(name='main', commit='def456', is_new=False)
            ],
            commits_count=3,
            files_changed=5,
            logs_copied=True,
    ...
    def test_count_files_changed(self):
        ...
        subprocess.run(['git', 'add', '.'], cwd=self.repo_path, check=True)
        subprocess.run(['git', 'commit', '-m', 'Add files'], cwd=self.repo_path, check=True, capture_output=True)
